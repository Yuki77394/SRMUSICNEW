...
class YouTubeAPI:
    ...
    async def download(
        ...
        lock_key, lock = await _get_download_lock(
            _download_lock_key(link), media_type
        )
        ...
        try:
            ...
            # Best-effort cleanup. Only remove an unlocked lock so another
            # waiter can never lose its shared lock while still using it.
            try:
                if not lock.locked():
