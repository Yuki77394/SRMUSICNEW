        ...
        if "live_" in queued:
            ...
            try:
                ...
                return await _safe_edit_or_send(
                    chat_id, original_chat_id, old_mystic, _["call_6"]
                )
            if not file_path:
                ...
                # Same _safe_edit_or_send recovery as above.
                LOGGER(__name__).warning(
                    f"change_stream: YouTube.download returned None for "
                    f"chat {chat_id} video={videoid} (silent failure)"
                )
                return await _safe_edit_or_send(
                    chat_id, original_chat_id, old_mystic, _["call_6"]
                )
